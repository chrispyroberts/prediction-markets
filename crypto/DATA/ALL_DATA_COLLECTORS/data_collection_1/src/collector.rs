...
impl BinanceBTCPerpetualDataCollector {
    pub async fn new(symbol: &str, display_mode: &str) -> Result<Self> {
    ...
    async fn process_orderbook_update(&mut self, data: &OrderBookUpdate, receipt_timestamp: i64) -> Result<()> {
        ...
        self.add_orderbook_to_batch(features.clone()).await?;
        // Update display
        self.update_display();
        Ok(())
    }
    ...
    async fn aggregate_and_add_to_batch(
        &self,
        trades: &[TradeInfo],
        trade_batch: &Arc<Mutex<Vec<AggregatedTrade>>>,
        trade_batch_size: usize,
        ...
        if batch.len() >= trade_batch_size {
            ...
            tokio::spawn(async move {
                match insert_aggregated_trades_batch(&client, &batch_to_write).await {
                    Ok(count) => info!("✅ Successfully wrote {} aggregated trades to TimescaleDB", count),
                    Err(e) => error!("Failed to insert trade batch: {}", e),
                }
            });
        }
    ...
    async fn add_orderbook_to_batch(&self, features: OrderBookFeatures) -> Result<()> {
        ...
        if batch.len() >= self.orderbook_batch_size {
            ...
            let client = self.db_client.clone();
            tokio::spawn(async move {
                match insert_orderbook_features_batch(&client, &batch_to_write).await {
                    Ok(count) => info!("✅ Successfully wrote {} orderbook features to TimescaleDB", count),
                    Err(e) => error!("Failed to insert orderbook batch: {}", e),
                }
            });
        }
        Ok(())
    ...
    pub async fn flush_remaining_batches(&self) -> Result<()> {
        // Orderbook
        {
            ...
            if !batch.is_empty() {
                let client = self.db_client.clone();
                let batch_to_write = batch.clone();
                insert_orderbook_features_batch(&client, &batch_to_write).await?;
            }
        }
        // Trades
        {
            ...
            if !batch.is_empty() {
                let client = self.db_client.clone();
                let batch_to_write = batch.clone();
                insert_aggregated_trades_batch(&client, &batch_to_write).await?;
            }
        }
        Ok(())
    }
    ...
    pub async fn run_with_restart(&mut self) -> Result<()> {
        ...
        // Always flush remaining batches before exiting, regardless of how we got here
        info!("RUN_COMPLETED | Flushing remaining batches before exit");
        if let Err(e) = self.flush_remaining_batches().await {
            error!("FLUSH_ERROR | Error flushing batches: {}", e);
        } else {
            info!("FLUSH_SUCCESS | All batches flushed successfully");
        }
