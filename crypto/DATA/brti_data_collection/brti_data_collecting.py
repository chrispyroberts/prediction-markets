...
class BRTIDataCollector:
    ...
    def setup_logging(self):
        ...
        # Create file handler
        log_file = 'logs/brti_backoff_analysis.log'
    ...
    def write_price_batch_to_parquet(self, batch_data):
        ...
        print(f"💾 BRTI BATCH: Wrote {len(batch_data)} records to Parquet")
    ...
    def add_price_to_batch(self, price, receipt_timestamp_ms):
        ...
        print(f"💰 BRTI QUEUED [{timestamp_est}] Price: ${price:,.2f} | Simple Avg: ${simple_avg:,.2f}")
    ...
    def test_connection(self, page):
        ...
        try:
            ...
            if not price_text or price_text.strip() == '':
                ...
                print("⚠️ Price element found but empty content - possible shadow ban")
                return False
            ...
            if price <= 0:
                ...
                print("⚠️ Invalid price value - possible shadow ban")
                return False
                
            self.log_connection_test(True)
            print(f"✅ Connection test successful - Price: ${price:,.2f}")
            return True
            
        except Exception as e:
            self.log_connection_test(False, str(e))
    ...
    def wait_for_retry(self, retry_count):
        ...
        print(f"⏳ Waiting {wait_time} seconds before retry #{retry_count}...")
            ...
            while retry_count < self.max_consecutive_retries:
                try:
                    ...
                    if not self.test_connection(page):
                        print("🚫 Initial connection test failed - possible shadow ban")
                        browser.close()
                        retry_count += 1
                        if retry_count < self.max_consecutive_retries:
                            self.wait_for_retry(retry_count)
                    ...
                    print("📡 Connected to BRTI page successfully")
                    ...
                    while True:
                        try:
                            ...
                            if not price_text or price_text.strip() == '':
                                ...
                                print(f"⚠️ Empty price text (failure #{consecutive_failures})")
                                
                                if consecutive_failures >= 5:
                                    self.log_consecutive_failures(consecutive_failures, "empty_price_text")
                                    print("🚫 Too many consecutive failures - possible shadow ban")
                                    break
                            ...
                            if price <= 0:
                                consecutive_failures += 1
                                print(f"⚠️ Invalid price: {price} (failure #{consecutive_failures})")
                                
                                if consecutive_failures >= 5:
                                    self.log_consecutive_failures(consecutive_failures, "invalid_price")
                                    print("🚫 Too many invalid prices - possible shadow ban")
                                    break
                            ...
                            if self.is_shadow_banned():
                                ...
                                print(f"🚫 Shadow ban detected - no price updates for {self.shadow_ban_timeout} seconds")
                                break

                        except Exception as e:
                            consecutive_failures += 1
                            print(f"⚠️ Error while fetching price (failure #{consecutive_failures}): {e}")
                            
                            if consecutive_failures >= 5:
                                self.log_consecutive_failures(consecutive_failures, "fetch_error", e)
                                print("🚫 Too many consecutive errors - possible shadow ban")
                                break
                    ...
                    retry_count += 1
                    if retry_count < self.max_consecutive_retries:
                        print(f"🔄 Attempting reconnection (retry #{retry_count}/{self.max_consecutive_retries})")
                        self.wait_for_retry(retry_count)
                    else:
                        self.logger.error(f"MAXIMUM_RETRIES_REACHED | Gave up after {self.max_consecutive_retries} attempts")
                        print(f"❌ Maximum retries ({self.max_consecutive_retries}) reached. Giving up.")
                        break
...
if __name__ == "__main__":
    ...
    print("📍 Timezone: EST")
    print("🛡️  Shadow ban detection enabled")
