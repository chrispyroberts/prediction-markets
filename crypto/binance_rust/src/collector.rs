...
impl BinanceBTCPerpetualDataCollector {
    pub fn new(symbol: &str, display_mode: &str) -> Result<Self> {
        let collector = Self {
            ...
            display_manager: DisplayManager::new(DisplayMode::from(display_mode)),
            ...
            orderbook_parquet: "better_data/btc_orderbook_features.parquet".to_string(),
            trade_parquet: "better_data/perp_trade_raw_data.parquet".to_string(),
    ...
    fn write_orderbook_batch_to_parquet(&self, batch_data: Vec<OrderBookFeatures>) -> Result<()> {
        ...
        if std::path::Path::new(&self.orderbook_parquet).exists() {
            // Read existing data
        ...
        if self.display_manager.get_mode() != &DisplayMode::Silent {
    ...
    fn update_display(&self) {
        ...
        let last_large_aggregated_trade = self.last_large_aggregated_trade.lock().as_ref().map(|t| t);
