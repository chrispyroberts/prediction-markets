...
from trading_utils import get_btc_vol_estimate, estimate_constract_price, estimate_constract_price_advanced, \
    estimate_price_using_model, get_binance_option_mark_price, get_time_to_expiry_hours, get_strike

mm = None  # Global market maker instance
shutdown_event = asyncio.Event()  # Global shutdown event

# === BRTI WebSocket Tracking ===
latest_brti_price = 100000
last_brti_update_time = None
brti_connected = False
brti_sio_client = None
BRTI_UPDATE_TIMEOUT = 10  # Seconds without update before halting trading

# === BTC Volatility Tracking ===
latest_btc_vol_estimate = None
latest_btc_vol_of_vol_estimate = None
last_btc_vol_update_time = None
...
def cleanup_and_exit():
    ...
    global mm, shutdown_event, brti_sio_client
    print("\n🛑 Program shutting down, cancelling all orders...")
    
    try:
        # Disconnect from BRTI WebSocket
        if brti_sio_client:
            try:
                brti_sio_client.disconnect()
                print("✅ Disconnected from BRTI WebSocket")
            except Exception as e:
                print(f"⚠️ Error disconnecting from BRTI WebSocket: {e}")
        
        if mm is not None:
            # Use the demo setting from the market maker
            # demo_mode = getattr(mm, 'demo', True)  # Default to True if not set
            result = cancel_all_orders(demo=True)
...
# === BRTI WebSocket Functions ===
def setup_brti_websocket():
    """Setup BRTI WebSocket client connection"""
    global brti_sio_client, brti_connected
    
    try:
        brti_sio_client = sio_client.Client()
        
        @brti_sio_client.event
        def connect():
            global brti_connected
            brti_connected = True
            print("🔗 Connected to BRTI WebSocket")
        ...
        @brti_sio_client.on('price_update')
        def on_brti_price_update(data):
            global latest_brti_price, last_brti_update_time, latest_btc_vol_estimate
            latest_brti_price = data.get('brti')
            vol = data.get('binance_option_price')
            if vol is not None:
                latest_btc_vol_estimate = vol

            else:

                print("❌ No BTC Volatility Estimate")
            last_brti_update_time = time.time()
            # print(f"💰 BRTI Update: ${latest_brti_price:,.2f} | Avg: ${data.get('simple_average', 0):,.2f}")
        
        # Connect to BRTI WebSocket
        brti_sio_client.connect('http://localhost:5001')
        print("📡 Connecting to BRTI WebSocket at http://localhost:5001...")
        
    except Exception as e:
        print(f"❌ Failed to connect to BRTI WebSocket: {e}")
        brti_connected = False

def is_brti_stale():
    """Check if BRTI price is stale (no update in last 10 seconds)"""
    global last_brti_update_time, BRTI_UPDATE_TIMEOUT
    
    if last_brti_update_time is None:
        return True
    
    time_since_update = time.time() - last_brti_update_time
    return time_since_update > BRTI_UPDATE_TIMEOUT

def get_brti_status():
    """Get current BRTI connection and price status"""
    global latest_brti_price, last_brti_update_time, brti_connected
    
    status = {
        'connected': brti_connected,
        'latest_price': latest_brti_price,
        'last_update_time': last_brti_update_time,
        'stale': is_brti_stale()
    }
    
    if last_brti_update_time:
        status['seconds_since_update'] = time.time() - last_brti_update_time
...
class MarketMaker:
    ...
    def check_brti_and_halt_trading(self):
        """Check BRTI status and halt/resume trading accordingly"""
        global latest_brti_price, last_brti_update_time
        
        brti_stale = is_brti_stale()
        
        if brti_stale and not self.trading_halted:
            # Halt trading - cancel all orders
            self.trading_halted = True
            self.last_halt_time = time.time()
            print(f"🚫 TRADING HALTED: BRTI price stale (no update for {BRTI_UPDATE_TIMEOUT}s)")
            print(f"   Last BRTI update: {time.time() - last_brti_update_time:.1f}s ago")
            print(f"   Latest BRTI price: ${latest_brti_price:,.2f}" if latest_brti_price else "   Latest BRTI price: None")
            
            # Cancel all orders
            try:
                result = cancel_all_orders(demo=self.order_manager.demo)
                if result['success']:
                    print(f"✅ Cancelled {result['cancelled_count']} orders due to BRTI staleness")
                else:
                    print(f"❌ Failed to cancel orders: {result.get('error', 'Unknown error')}")
            except Exception as e:
                print(f"❌ Error cancelling orders: {e}")
        
        elif not brti_stale and self.trading_halted:
            # Resume trading
            self.trading_halted = False
            halt_duration = time.time() - self.last_halt_time if self.last_halt_time else 0
            print(f"✅ TRADING RESUMED: BRTI price updated (halted for {halt_duration:.1f}s)")
            print(f"   Current BRTI price: ${latest_brti_price:,.2f}")
    ...
    async def update_quote(self, ticker):
        ...
        """
        # Check BRTI status first - halt trading if stale
        if self.check_brti_and_halt_trading():
            # Trading is halted, don't update quotes
            self.our_quotes[ticker]['bid'] = None
            self.our_quotes[ticker]['ask'] = None
            return
    ...
    def calculate_unrealized_pnl(self, ticker):
        """Calculate unrealized PnL for a position given current market price."""
        if ticker not in self.positions or ticker not in self.avg_prices:
            return 0.0
        ...
        # calculate estimated price for the contract
        estimated_price = estimate_price_using_model(ticker, latest_brti_price, latest_btc_vol_estimate)

            
        if position > 0:
            # Long position: PnL = (current_price - avg_entry) * quantity
            unrealized = (estimated_price - avg_price) * position
        else:
            # Short position: PnL = (avg_entry - current_price) * abs(quantity)
            unrealized = (avg_price - estimated_price) * abs(position)
    ...
    def get_portfolio_summary(self):
        ...
        for ticker in self.tickers:
            if self.positions[ticker] != 0:
                self.calculate_unrealized_pnl(ticker)
...
async def periodic_status_report():
    ...
    while True:
        ...
        # Get BRTI status
        brti_status = get_brti_status()
        print(f"💰 BRTI Status:")
        print(f"   Connected: {brti_status['connected']}")
        print(f"   Latest Price: ${brti_status['latest_price']:,.2f}" if brti_status['latest_price'] else "   Latest Price: None")
        print(f"   Stale: {brti_status['stale']}")
        if brti_status['last_update_time']:
            print(f"   Last Update: {brti_status['seconds_since_update']:.1f}s ago")
        print(f"   Trading Halted: {mm.trading_halted}")
        
        # Get BTC Volatility status
        print(f"📊 BTC Volatility Estimate:")
        if latest_btc_vol_estimate is not None:
            print(f"   Latest Estimate: {latest_btc_vol_estimate:.2%}")
        else:
            print(f"   Latest Estimate: None")
        if last_btc_vol_update_time:
            seconds_since_vol_update = time.time() - last_btc_vol_update_time
            print(f"   Last Update: {seconds_since_vol_update:.1f}s ago")
        ...
        for ticker in mm.tickers:
            try:
                estimated_price = estimate_price_using_model(ticker, latest_brti_price, latest_btc_vol_estimate)
                # estimated_price = estimate_constract_price_advanced(ticker, latest_brti_price, latest_btc_vol_estimate, latest_btc_vol_of_vol_estimate)
                ...
                if not(live_bid is None or live_ask is None):
                    ...
                    market_bid_size = yes_book[market_bid] if market_bid else 0
                    market_ask_size = no_book[market_ask] if market_ask else 0
                    
             
                    print(f"    {ticker}:")
                    print(f"    Estimated: {estimated_price:.2f}%")
                    print(f"    Market: {market_bid}/{market_ask} (qty {market_bid_size}/{market_ask_size})")
...
def get_snapshot():
    ...
    for ticker in tickers:
        ...
        if tte is None:
            tte = get_time_to_expiry_hours(ticker)

        # just send top orderbook yes and no
        yes_orderbook = mm.orderbooks[ticker]['yes']
        no_orderbook = mm.orderbooks[ticker]['no']
        ...
        best_bid_qty = 0
        i = 0
        for price, qty in sorted(yes_orderbook.items(), reverse=True):
            if i == 0:
        ...
        best_ask_qty = 0
        i = 0
        for price, qty in sorted(no_orderbook.items()):
            if i == 0:
...
def get_broadcast_payload():
    orderbooks, tte = get_snapshot()
    return {
        'timestamp': time.time(),
        'brti_price': latest_brti_price,
...
def background_broadcast():
    while True:
        ...
        socketio.emit('orderbook_update', payload, namespace='/orderbook')
        time.sleep(5)

@socketio.on('connect', namespace='/orderbook')
def on_connect():
    print('Client connected to /orderbook namespace')

@socketio.on('disconnect', namespace='/orderbook')
def on_disconnect():
    print('Client disconnected from /orderbook namespace')

# Start background thread for broadcasting after mm is initialized
...
if __name__ == "__main__":
    ...
    series = "KXBTCD" # normal calls!
    # series = "KXBTC"  # ranges calls
    event = get_current_event(series)
    ...
    # Setup BRTI WebSocket connection
    print("Setting up BRTI WebSocket connection...")
    setup_brti_websocket()
    
    # Get initial BTC volatility estimate    
    ...
    threading.Thread(target=lambda: socketio.run(app, host="0.0.0.0", port=5010), daemon=True).start()
